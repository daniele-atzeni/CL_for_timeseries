...
class Objective:
    def __init__(
        ...
        self.train, self.test, self.freq, self.seasonality = get_dataset_from_file(
            f"{ROOT_FOLDER}/tsf_data/{DATASET_NAME}", prediction_length, context_length
        )
        self.model = MODEL
        ...
        NORMALIZER_NAME = (
            "gas_t_student"  # "gas_simple_gaussian", "gas_complex_gaussian"
        )
        ...
        ROOT_FOLDER = f"RESULTS_{DATASET_NAME}_{NORMALIZER_NAME}_{MEAN_LAYER_NAME}_{DL_MODEL_LIBRARY}"
        ...
        if self.model == "ffn":
            return {
                "num_hidden_dimensions": [
                    trial.suggest_int("hidden_dim_{}".format(i), 10, 100)
                    for i in range(trial.suggest_int("num_layers", 1, 5))
                ],
                "trainer:learning_rate": trial.suggest_loguniform(
                    "trainer:learning_rate", 1e-5, 1e-1
                ),
            ...
            valid_pairs = [
                (i, d) for i in range(10, 101) for d in range(1, 11) if i % d == 0
            ]
            model_dim_num_heads_pair = trial.suggest_categorical(
                "model_dim_num_heads_pair", valid_pairs
            )
    ...
    def __call__(self, trial):
        ...
        if self.model == "ffn":
            estimator = FF_gluonts(
                ...
                num_hidden_dimensions=params[
                    "num_hidden_dimensions"
                ],  # num_hidden_dimensions,
                trainer=Trainer(
                    ctx=self.ctx,
                    epochs=params["trainer:epochs"],
                    learning_rate=params["trainer:learning_rate"],
        ...
        for item_id, ts in enumerate(self.test):
            ...
            mae_naive = mean_absolute_error(
                np.array(training_data)[int(self.seasonality) :],
                y_pred_naive,
...
def run(
    ...
    study.optimize(
        Objective(
            ...
            context_length,
            ctx,
            multivariate=False,  # multivar seems 0.02-0.03 better
        ),
        n_trials=n_trials,
    )
    ...
    with open(file_path, "a") as file:
        file.write(
            f" ########################### {model_choice} with gas norm on {DATASET_NAME} Final MASE: {trial.value}\n"
        )
