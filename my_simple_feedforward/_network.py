...
class SimpleFeedForwardNetworkBase(mx.gluon.HybridBlock):
    ...
    def get_distr_args(
        self, F, past_target: Tensor, proc_means: Tensor
        ...
        # here we have to change the output of the network
        # ##############
        # we must include the means
        mlp_outputs = mlp_outputs + proc_means
        ##############
...
class SimpleFeedForwardTrainingNetwork(SimpleFeedForwardNetworkBase):
    ...
    def __init__(
        ...
        denormalizer,
        *args,
        **kwargs,  # self, normalizer: Normalizer, denormalizer: Denormalizer, *args, **kwargs
    ) -> None:
    ...
    def hybrid_forward(
        ...
        """
        ############
        # real_n_features = past_target.shape[-1] // 2
        real_n_features = 414
        # shape_mean = (100, 100, 414)

        proc_data = past_target.slice(begin=(0, 0, 0), end=(-1, -1, real_n_features))
        mean_data = past_target.slice(begin=(0, 0, real_n_features), end=(-1, -1, -1))
        lin, bias = list(self.denormalizer.get_mus_params())
        lin = mx.nd.array(lin.detach().numpy())
        bias = mx.nd.array(bias.detach().numpy())
        shape_mean = mx.symbol.shape_array(mean_data)
        mean_data = mx.symbol.flatten(mean_data)
        proc_means = mx.symbol.add_n(mx.symbol.linalg.gemm2(lin, mean_data), bias)
        ############
        distr_args, loc, scale = self.get_distr_args(F, proc_data, proc_means)
